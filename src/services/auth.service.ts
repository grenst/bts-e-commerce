...
import { signupCustomer, fetchMyCustomer, CustomerDraft } from '../components/auth-services/customer.service';
...
function persistTokens(token: OAuthTokenResponse) {
  ...
  useTokenStore.getState().setTokens(access_token, refresh_token ?? null, expires_in);
}

export class AuthService {
  static async register(email: string, password: string, firstName?: string, lastName?: string): Promise<boolean> {
    try {
      // anonymous token /me/signup
      const anon = await getAnonymousToken();
      await signupCustomer({ email, password, firstName, lastName } as CustomerDraft, anon.access_token);
      ...
      // UX‑msg
      useUIStore.getState().addNotification('error', `Registration failed: ${(error as Error).message}`);
      return false;
    }
  }
  ...
  static async login(email: string, password: string): Promise<boolean> {
    try {
      ...
      useUIStore.getState().addNotification('info', `Access‑token:\n${token.access_token}`, 10000);

      return true;
    } catch (error) {
      debug('Login error', error);
      await this.logout();
      useUIStore.getState().addNotification('error', 'Login failed. Check credentials.');
      return false;
    }
  }
