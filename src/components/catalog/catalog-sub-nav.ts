...
export function createCatalogSubNavElement(): CatalogSubNavControl {
  ...
  const renderFilterPills = () => {
    ...
    contentElement.append(buildPill('All', !selectedCategoryId, () => {
    ...
    for (const category of categories) {
      const name = category.name.en || Object.values(category.name)[0] || 'Unnamed'
    ...
    for (const size of SIZE_VALUES) {
      ...
      contentElement.append(
        buildPill(size, active, () => {
          active ? selectedSizes.delete(size) : selectedSizes.add(size)
          renderFilterPills()
          dispatchFiltersChanged()
        }),
      )
  ...
  const renderSortOptions = () => {
    ...
    for (const option of SORT_OPTIONS) {
      ...
      sortButton.addEventListener('click', () => {
        ...
        renderSortOptions()
        const ev = new CustomEvent('sort-changed', {
  ...
  const toggle = (mode: 'filters' | 'sort') => {
    currentMode === mode ? hide() : show(mode)
  }
