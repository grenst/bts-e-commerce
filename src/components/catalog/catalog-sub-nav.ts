...
interface CatalogSubNavControl {
  ...
  getCurrentMode: () => 'filters' | 'sort' | null;
}
...
export function createCatalogSubNavElement(): CatalogSubNavControl {
  ...
  let currentMode: 'filters' | 'sort' | null = null;
  const contentElement = createEl({
    tag: 'div',
  ...
  const renderFilterPills = () => {
    ...
    if (!categories || categories.length === 0) {
      const noCategoriesEl = createEl({
        tag: 'p',
        classes: ['text-gray-600', 'text-center', 'w-full'],
        text: 'No categories available'
      });
      contentElement.appendChild(noCategoriesEl);
      return;
    }
    ...
    categories.forEach(category => {
      ...
      const categoryName = category.name.en || Object.values(category.name)[0] || 'Unnamed Category';
  ...
  const renderSortOptions = () => {
    ...
    SORT_OPTIONS.forEach(option => {
      ...
      if (isActive) {
        ...
        sortBtn.appendChild(arrowImg);
      }

      sortBtn.addEventListener('click', () => {
        if (activeSortMode.key === option.key) {
          // Toggle direction if same option clicked
          activeSortMode = { ...activeSortMode, asc: !activeSortMode.asc };
        } else {
          // Set new sort option with default ascending direction
          activeSortMode = { key: option.key, asc: true };
        }
      ...
      contentElement.appendChild(sortBtn);
    });
  };
      ...
      if (categories) {
        ...
        getAllCategories()
          .then((fetchedCategories) => {
            ...
            categories.forEach(cat => activeCategoryIds.add(cat.id));
            renderFilterPills();
          })
          .catch((error: Error) => {
            ...
            contentElement.appendChild(errorEl);
          });
      }
    } else {
      renderSortOptions();
